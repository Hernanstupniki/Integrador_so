    ...
    for i in range(NUMERO_PAGINAS):
        if paginas_memoria[i] is None:
            ...
            if paginas_asignadas == paginas_necesarias:
                return True  # Se asignaron todas las páginas necesarias

    # Si no se pudieron asignar todas las páginas necesarias, liberar las asignadas
    if paginas_asignadas < paginas_necesarias:
        liberar_paginas(proceso)
        return False  # No se pudo asignar memoria suficiente
...
def mostrar_procesos_en_memoria():
    ...
    # Agrega una lógica para manejar asignaciones discontiguas
    procesos_dibujados = set()
    for i in range(NUMERO_PAGINAS):
        proceso_id = paginas_memoria[i]
        if proceso_id and proceso_id not in procesos_dibujados:
            procesos_dibujados.add(proceso_id)
            # Encuentra los rangos de las asignaciones del proceso y dibújalos como bloques individuales
            inicio = i
